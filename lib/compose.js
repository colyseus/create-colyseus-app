...
function compose({ outputDir, baseDir, language, fragments, vars = {} }) {
  ...
  substituteVars(outputDir, {
    ...vars,
    // Points docs at files wherever the shape put them ("" or "apps/backend/").
    // POSIX separators even on Windows: this lands in prose, not in an fs call.
    "app-prefix": appRoot === outputDir ? "" : `${path.relative(outputDir, appRoot).replaceAll(path.sep, "/")}/`,
